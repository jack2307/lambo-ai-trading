...
//! # Semantics
//!
//! [`in_blackout`]`(t, before, after, min_impact)` is true when some
//! installed event with `impact >= min_impact` satisfies
//! `event.time - before <= t < event.time + after` — closed at the front,
//! open at the back, so a bar stamped exactly `after` ms past the release is
//! already tradable. The `t` a filter passes is the **signal bar's open
//! time**; the fill is the next bar's open (the engine's rule), so a bar that
//! opens just before the window and would fill inside it is *not* blocked.
//! Pad `before` by one bar if that matters to a hypothesis.
...
/// `impact >= min_impact`. False when nothing is installed.
#[must_use]
pub fn in_blackout(t_ms: i64, before_ms: i64, after_ms: i64, min_impact: u8) -> bool {
    blackout_in(events(), t_ms, before_ms, after_ms, min_impact)
}
...
#[must_use]
pub fn blackout_in(events: &[NewsEvent], t_ms: i64, before_ms: i64, after_ms: i64, min_impact: u8) -> bool {
    let first = events.partition_point(|e| e.time <= t_ms - after_ms);
    events[first..].iter().take_while(|e| e.time - before_ms <= t_ms).any(|e| e.impact >= min_impact)
}
...
pub(crate) fn test_events() -> Vec<NewsEvent> {
    ...
    // 2026-09-14 (a Monday): high at 12:30 UTC (the NFP/CPI slot) and at
    // 15:00 UTC, far enough apart that their 60/30 windows do not touch;
    // medium at 09:00 UTC.
    let day = days_from_civil(2026, 9, 14) * 86_400_000;
    vec![
        NewsEvent { time: day + 15 * 3_600_000, impact: 3, currency: "USD".into() },
        NewsEvent { time: day + 12 * 3_600_000 + 30 * 60_000, impact: 3, currency: "USD".into() },
        NewsEvent { time: day + 9 * 3_600_000, impact: 2, currency: "EUR".into() },
    ]
}
...
mod tests {
    ...
    fn ev(time: i64, impact: u8) -> NewsEvent {
        NewsEvent { time, impact, currency: "USD".into() }
    }
    ...
    fn the_window_is_closed_before_and_open_after() {
        ...
        let hit = |t: i64, min_impact: u8| blackout_in(&events, t, before, after, min_impact);
    ...
    fn min_impact_selects_which_events_count() {
        ...
        assert!(!blackout_in(&events, 1950 * MIN, before, after, 3));
        assert!(blackout_in(&events, 1950 * MIN, before, after, 2));
        assert!(blackout_in(&events, 1950 * MIN, before, after, 1));
        // 2020 min is inside both the medium event's [1940, 2030) and the
        // high one's [1970, 2060): blocked at every threshold.
        assert!(blackout_in(&events, 2020 * MIN, before, after, 3));
        // 2040 min: the medium window has closed, the high one is open.
        assert!(blackout_in(&events, 2040 * MIN, before, after, 3));
        // 2070 min: both closed.
        assert!(!blackout_in(&events, 2070 * MIN, before, after, 1));
    }

    #[test]
    fn an_empty_list_never_blocks() {
        assert!(!blackout_in(&[], 0, 60 * MIN, 30 * MIN, 1));
        assert!(!blackout_in(&[], i64::MAX / 2, 0, 0, 0));
    }
    ...
    fn zero_widths_block_only_the_instant_before_the_release() {
        ...
        assert!(!blackout_in(&events, 1000 * MIN, 0, 0, 3));
        // before = 0, after = 1 ms: only the release instant.
        assert!(blackout_in(&events, 1000 * MIN, 0, 1, 3));
        assert!(!blackout_in(&events, 1000 * MIN - 1, 0, 1, 3));
    }
    ...
    fn the_global_path_reads_the_installed_list() {
        ...
        assert!(in_blackout(release - 60 * MIN, 60 * MIN, 30 * MIN, 3));
        assert!(!in_blackout(release - 60 * MIN - 1, 60 * MIN, 30 * MIN, 3));
        assert!(!in_blackout(release + 30 * MIN, 60 * MIN, 30 * MIN, 3));
        // The 09:00 EUR event is medium: invisible at the default threshold.
        let medium = test_events()[2].time;
        assert!(!in_blackout(medium, 60 * MIN, 30 * MIN, 3));
        assert!(in_blackout(medium, 60 * MIN, 30 * MIN, 2));
    }
